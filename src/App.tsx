            ...
            {isAdmin && (
              <div className="flex items-center gap-1 bg-amber-100/60 rounded-lg p-0.5 mt-1">
                ...
                <button
                  ...
                  {finalizedRecords.filter(r => !r.is_deleted).length > 0 && (
                    <span className="text-[9px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-bold">
                      {finalizedRecords.filter(r => !r.is_deleted).length}
                    </span>
                  )}
                </button>
              </div>
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            {isAdmin && viewFinalized ? (
              ...
              <div className="flex-1 overflow-y-auto">
                {/* Filter buttons */}
                <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 py-2 flex items-center gap-1.5">
                  {(['all', 'finalized', 'deleted'] as const).map(f => (
                    <button key={f} onClick={() => setFinalizedFilter(f)}
                      ...
                      {f === 'all' ? `All (${finalizedRecords.length})` : f === 'finalized' ? `Finalized (${finalizedRecords.filter(r => !r.is_deleted).length})` : `Deleted (${finalizedRecords.filter(r => r.is_deleted).length})`}
                    </button>
                  ))}
                </div>
                {finalizedLoading ? (
                      ...
                      <div key={rec.id} onClick={() => setSelectedFinalizedId(rec.source_doctor_id)}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            {!rec.is_deleted && rec.location_count > 0 && (
